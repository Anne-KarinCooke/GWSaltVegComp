...
OF<- function(h, soilpar){
  ...
  qq=(soilpar$cn/soilpar$Mn)*h^(5/3)*soilpar$Sl^(1/2)
...
balances2D <- function(Rain, par,plotit=T,
  ...
  #mass balance
  mb <- matrix(0,nrow= length(Rain), ncol =grids)
  ...
  Diff_sub<- matrix(0,nrow=deltat, ncol =grids) # divergence of soil moisture from one grid cells to the next
  ...
  mb_sub<-matrix(0,nrow= deltat, ncol =grids)
...
#        q_sub[tt,g]<-ifelse(g>1,OF(h=h.old[g],soilpar=soilpar),0)  ## g=1 is on top of the hill and does not receive any runoff from another cell
        
        q_sub[tt,g]<-OF(h=h.old[g],soilpar=soilpar)  ## g=1 is on top of the hill and does not receive any runoff from another cell
...
#                h_sub[tt+1,g] <- h.old[g] + ifelse(tt==1,Rain[t],0) - Infil(h.old[g], P.old[g],par)*timeincr) ## g=1 is on top of the hill and does not receive any runoff from another cell
        ...
        h_sub[tt+1,g] <- ifelse(g>1, h.old[g] + ifelse(tt==1,Rain[t]+q_sub[tt,g-1],q_sub[tt, g-1]) - Infil(h.old[g], P.old[g],par)*timeincr,
               h.old[g] + ifelse(tt==1,Rain[t],0) - Infil(h.old[g], P.old[g],par)*timeincr) ## g=1 is on top of the hill and does not receive any runoff from another cell


   
        # Infiltration
       # ifelse(h_sub[tt+1,g]<soilpar$K_s, soilpar$alpha_i <- 1, soilpar$alpha_i<- (1-(h_sub[tt+1,g]-soilpar$K_s)/h_sub[tt+1,g]))
        par$alpha_i <- ifelse(h_sub[tt+1,g]<soilpar$K_s*timeincr, 1,(1-(h_sub[tt+1,g]-soilpar$K_s*timeincr)/h_sub[tt+1,g]))
       ...
       mb_sub[tt,g] <- I_sub[tt,g] - WU_sub[tt,g] + flux_sub[tt,g] - q_sub[tt,g]
  ...
  Out <- data.frame(P=P,M=M,h=h, CM=CM, SmM=SmM, In=In, flux=flux, Svir=Svir,h=h, q=q, mb=mb)
  return(Out)
}
