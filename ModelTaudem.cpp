...
// [[Rcpp::export]]
mat Subsurface(int ro, int co, int border, mat flowdir, mat flowdirTable, mat M, mat filler, double Dm, double timeincr){
  //flowdirTable
...
// [[Rcpp::export]]
List SurfaceSoilSaltWBGRID(int border, Rcpp::List soilpar, Rcpp::List vegpar, Rcpp::List saltpar,
      ...
      for (j=border; j< (cols-border); j++ ){
        ...
        for (tt = 0; tt< (deltat-1); tt++){
          ...
          if (((h_sub(i,j,tt+1) + elev(i,j)*1000.0) > (diff(i,j)*1000.0)) & (diff(i,j) > 0.0)){ // times 1000 for [mm]
            ...
            runon_store = Surface(rows, cols, border, flowdir_adj, write_flowdirTable(), q_sub.slice(tt+1), ones);
            runon_sub(i,j,tt+1) = runon_store(i,j);
