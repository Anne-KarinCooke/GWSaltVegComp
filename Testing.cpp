...
List balances2D(int t, NumericVector Rain,
  ...
  // INNER LOOP (SUbDAILY) function
        ...
        for (tt==1; tt< deltat; tt++){ 
          ...
          // Surface Balance 
          SurfaceWB(i,j,tt, t_old, alpha_i, timeincr,cn, Mn, Rain){
          ...
          // Extra Water Balance Function
          
          WB_cpp(i,j,tt, t_old, vegpar, soilpar){
          ...
          // salt leaching
          // 
          SaltBalance_cpp (flux, saltpar, timeincr){
            // //CM sub
            double CM_sub[rows][cols][deltat];
            // // SmIsub
            double SmI_sub[rows][cols][deltat];
