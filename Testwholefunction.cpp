...
// [[Rcpp::export]]
List balances2D( NumericVector Rain, int alpha_i=1, double Zras = 3000.0, double rn = 1.0,
                 double slope=0.001){
  ...
  //
  // double rn[rows][cols] = {{1.0}};
  // double Zras[rows][cols] = {{200.0}};
    ...
    for (j=1; j< cols; j++ ){
      ...
      for (t=1; t< time; t++){
        ...
        for (tt=1; tt< time; tt++){
          ...
          runon_sub[i][j][tt] = rn*q_sub[i][j][t_old];
