...
# plot(rasterObj)
# 
# ter<-terrain(rasterObj, opt='flowdir', unit='radians', neighbors=8)
# 
# plot(ter)
# ter
# values(ter)
# # 
# # 
# sl<-seq(0,3,length.out=10)
# sl
# 
# df<- c(sl, 0.3*sl, sl, sl, 2*sl, 5*sl, sl, sl, sl,sl)
# df
# 
# rasterObj <- raster(ncol=10, nrow=10, xmn=0, xmx=10, ymn=0, ymx=10)
# 
# values(rasterObj) <- df
# plot(rasterObj)
# ter<-terrain(rasterObj, opt='flowdir', unit='radians', neighbors=8)
