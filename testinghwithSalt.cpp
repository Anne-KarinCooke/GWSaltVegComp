    ...
    for (j=0; j< cols; j++ ){
      ...
      for (t = 1; t< time; t++){
        ...
        for (tt = 0; tt< (deltat); tt++){
          ...
          Rcpp::Rcout <<  I_sub[i][j][tt];
          
          WU_sub[i][j][tt] = WU(M_sub[i][j][tt], P_sub[i][j][tt], gmax_in, k1_in) * 0.833333; //Svir for M
          
          M_sub[i][j][tt+1] = M_sub[i][j][tt] + I_sub[i][j][tt] - WU_sub[i][j][tt];
          // 
          Gr_sub[i][j][tt] = Gr(M_sub[i][j][tt], P_sub[i][j][tt], c_in, gmax_in, k1_in) * 0.833333; //Svir for M
          //  // // // //Mortality
          Mo_sub[i][j][tt] = Mo(P_sub[i][j][tt], M_sub[i][j][tt], M_sub[i][j][tt],d_in) * 0.833333;  //Svir for 2nd M
          //  // // //
          ...
          //
          M_sub[i][j][tt+1] = M_sub[i][j][tt] +  (flux_sub[i][j][tt] * 0.833333); 
        ...
        h[i][j][t+1] = h_sub[i][j][deltat];
        Wu[i][j][t+1] = WU_sub[i][j][deltat];
        P[i][j][t+1] = P_sub[i][j][deltat];
        M[i][j][t+1] = M_sub[i][j][deltat];
        CM[i][j][t+1] = CM_sub[i][j][deltat];
        SmI[i][j][t+1] = SmI_sub[i][j][deltat];
        SmM[i][j][t+1] = SmM_sub[i][j][deltat];
        Svir[i][j][t+1] = Svir_sub[i][j][deltat];
        ...
        for(int tt = 0; tt < deltat; tt++)
        {
          ...
          sumq += q_sub[i][j][tt];
          sumflux += flux_sub[i][j][tt] * 0.833333;
          summb += mb_sub[i][j][tt];
        }
...
soilpar_in <- soil_simple()
  ...
  SurfaceSoilSaltWBGRID(alpha_i =1.0, cn=0.01, Mn=0.04, Rain=1.0, slope=0.001,Zras=2000.0, soilpar=soilpar_in, vegpar=vegpar_in,saltpar=saltpar_in)
# Grid_run()
  */